...
public class OrderService {
                ...
                .map(order -> {
                    return OrderResponse.builder()
                            ...
                            .pickupImage(order.getPickupImage() != null
                                    ? Base64.getEncoder()
                                            .encodeToString(ImageUtils.decompressImage(order.getPickupImage()))
                                    : null)

                            .deliveryImage(order.getDeliveryImage() != null
                                    ? Base64.getEncoder()
                                            .encodeToString(ImageUtils.decompressImage(order.getDeliveryImage()))
                                    : null)
    ...
    public List<OrderByManagerResponse> getByManager() {
        ...
        responses.addAll(destinationOrders.stream()
                .sorted(Comparator.comparing(Order::getCreatedAt).reversed())
                .map(order -> buildOrderByManagerResponse(order, warehouse, false)) // false: manager từ destination
                                                                                    // warehouse
                .collect(Collectors.toList()));
    ...
    private OrderByManagerResponse buildOrderByManagerResponse(Order order, WarehouseLocations managerWarehouse,
            boolean isManagingSourceWarehouse) {
        ...
        if ((order.getStatus() == OrderStatus.CREATED
                ...
                || order.getStatus() == OrderStatus.RECEIVED_AT_SOURCE
                || order.getStatus() == OrderStatus.AT_DESTINATION)) {
            warehouseManagerRole = isSourceWarehouseFlag ? "sourceWarehouseManager" : "destinationWarehouseManager";
        } else {
            warehouseManagerRole = !isSourceWarehouseFlag ? "destinationWarehouseManager" : "sourceWarehouseManager";
        }
